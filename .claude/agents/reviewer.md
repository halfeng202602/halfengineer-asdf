...
8. **仕様との乖離** — 実装が仕様と異なる箇所がある場合、以下を判定する：
   - 実装が正しく仕様が古い → `[SPEC_UPDATE_NEEDED]` として報告。仕様側の更新を推奨
   - 仕様が正しく実装が間違い → 通常のCritical/Major指摘として報告

> 設計根拠: 「失敗したとき、問いは"誰がこのコードを間違えたか"ではなく
> "仕様が間違っていたのか、実装が間違っていたのか"になる。
> どちらの答えもアクション可能だ」（arXiv 2602.00180 解説, 2026）
...
## Spec Feedback（仕様への書き戻し推奨）
- [SPEC_UPDATE_NEEDED] [対象ファイル] {仕様のどこをどう更新すべきか}
