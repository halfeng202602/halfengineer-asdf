...
4. **reviewer の Spec Feedback（SPEC_UPDATE_NEEDED）がある場合、同様に判定して書き戻す**
5. documenter に成果物を渡してドキュメント更新を委任する
6. 承認ポイントならPOに報告して承認を待つ
7. 承認ポイントでなければ次のフェーズに進む
...
仕様フィードバック:
  - 書き戻し件数: {N件}
  - 対象ファイル: {requirements.md / design.md}
  - 詳細: specs/{feature}/requirements.md の「実装時補記」参照
